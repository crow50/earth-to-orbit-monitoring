...
# Migrations
.PHONY: migrate-local
migrate-local: ## Run database migrations locally via migrate service
...
deploy-prod: ## Production deployment workflow (Dry run by default)
	...
	@if [ "$(DRY_RUN)" = "1" ]; then \
		echo "--- DRY RUN ---"; \
		echo "ssh $(DEPLOY_HOST) 'cd $(DEPLOY_DIR) && git pull --ff-only && $(COMPOSE_CMD) pull && $(COMPOSE_CMD) build --pull && $(COMPOSE_CMD) run --rm migrate && $(COMPOSE_CMD) up -d && $(COMPOSE_CMD) ps'"; \
	else \
		echo "Executing deployment..."; \
		ssh $(DEPLOY_HOST) "cd $(DEPLOY_DIR) && git pull --ff-only && $(COMPOSE_CMD) pull && $(COMPOSE_CMD) build --pull && $(COMPOSE_CMD) run --rm migrate && $(COMPOSE_CMD) up -d && $(COMPOSE_CMD) ps"; \
	fi
